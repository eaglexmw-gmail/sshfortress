<template>
    ...
    >
        <el-form :model="form" :label-width="$store.state.dialogFormLabelWidth" :rules="rules" ref="ruleForm"
                 status-icon>
            <el-form-item :label="$t('clusterJumperName')" prop="name">
            ...
            <el-form-item :label="$t('clusterJumperSshAddr')" prop="ssh_addr">
                <el-input v-model.trim="form.ssh_addr" autocomplete="off" :disabled="isView"></el-input>
            </el-form-item>

            <el-form-item :label="$t('clusterJumperSshPort')" prop="ssh_port">
                <el-input v-model.number="form.ssh_port" autocomplete="off" :disabled="isView"></el-input>
            </el-form-item>
            ...
            <el-form-item :label="$t('sshPassword')" prop="password" v-show="form.ssh_type === 'password'">
                <el-input type="password"
                          prefix-icon="el-icon-c-scale-to-original"
                          show-password
                          v-model.trim="form.input_ssh_password" autocomplete="off"
                          :disabled="isView"></el-input>
            </el-form-item>

            <el-form-item :label="$t('sshKey')" prop="input_ssh_key" v-show="form.ssh_type === 'key'">
                <el-input v-model.trim="form.input_ssh_key" type="textarea" :rows="5" :placeholder="$t('sshKeyPhS')"
                          :disabled="isView"
                          maxlength="2048"
                          show-word-limit
            ...
            <el-form-item :label="$t('sshKeyPassword')" prop="input_ssh_key_password" v-show="form.ssh_type === 'key'">
                <el-input type="password"
                          ...
                          maxlength="20"
                          show-word-limit
                          v-model.trim="form.input_ssh_key_password" autocomplete="off"
                          :disabled="isView"></el-input>
            </el-form-item>
    ...
    export default {
        ...
        computed: {
            dTitle() {
                switch (this.type) {
                    case "create":
                        return this.$t('clusterJumperCreate');
                    case "view":
                        return this.$t('clusterJumperView');
                    case "edit":
                        return this.$t('clusterJumperUpdate');
                    default:
                        return ""
                }
            },
        ...
        methods: {
            ...
            doOpen() {
                ...
                if (this.isView && this.id > 0 && !!this.obj) {
                    this.$http.get(`cluster-jumper/${this.id}`).then(resp => {
                        if (resp) {
                            this.form = resp.data
                        }
                    })
