...
<script>
    export default {
        ...
        computed: {
            dTitle() {
                return this.$t('clusterSshBindMachine', this.obj)
            },
        },
        methods: {
            doOpen() {
            ...
            handleFormSubmit() {
                let cluster_ssh_id = this.obj.id;
                let machine_ids = this.value;

                this.$http.post("cluster-ssh-bind", {cluster_ssh_id, machine_ids}).then(res => {
                    if (res) {
